...
import { invoices, invoiceItems, invoicePayments, customers, jobs, teams } from "../db/schema.js";
...
router.get("/", async (req, res) => {
    try {
        ...
        if (customerId) {
            whereConditions.push(eq(invoices.customerId, parseInt(customerId)));
        }
        if (dateFrom) {
            whereConditions.push(gte(invoices.issueDate, dateFrom));
        }
        if (dateTo) {
            whereConditions.push(lte(invoices.issueDate, dateTo));
        }
...
router.post("/", async (req, res) => {
    try {
        ...
        const jobsToInvoice = await db
            ...
            .leftJoin(teams, eq(jobs.teamId, teams.id))
            .where(and(inArray(jobs.id, jobIds), eq(jobs.status, "completed")));
...
router.post("/:id/payments", async (req, res) => {
    try {
        ...
        const totalPayments = await db
            .select({ totalPaid: sql `COALESCE(SUM(${invoicePayments.amount}), 0)` })
            .from(invoicePayments)
            .where(eq(invoicePayments.invoiceId, invoiceId));
...
router.get("/jobs/available", async (req, res) => {
    try {
        ...
        if (customerId) {
            whereConditions.push(eq(jobs.customerId, parseInt(customerId)));
        }
        ...
        const availableJobs = await db
            ...
            .innerJoin(customers, eq(jobs.customerId, customers.id))
            .leftJoin(teams, eq(jobs.teamId, teams.id))
            .where(and(eq(jobs.status, "completed"), sql `${jobs.id} NOT IN (
