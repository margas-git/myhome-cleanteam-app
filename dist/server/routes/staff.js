...
router.post("/time-entries/clock-in", async (req, res) => {
    try {
        ...
        const currentTeamMembers = await db
            ...
            .where(and(eq(teamsUsers.teamId, teamId), lte(teamsUsers.startDate, new Date().toISOString().split('T')[0]), or(isNull(teamsUsers.endDate), gte(teamsUsers.endDate, new Date().toISOString().split('T')[0]))));
