 ...
 */
export async function getTeamMembersAtDate(teamId: number, date: Date) {
  ...
  const teamMembers = await db
    ...
    .where(
      and(
        ...
        lte(teamsUsers.startDate, dateString),
        or(
          isNull(teamsUsers.endDate),
          gte(teamsUsers.endDate, dateString)
        )
      )
    );
 ...
 */
export async function getUserTeamsAtDate(userId: number, date: Date) {
  const dateString = date.toISOString().split('T')[0]; // Convert to YYYY-MM-DD format
 ...
 */
export async function getCurrentTeamMembers(teamId: number) {
 ...
 */
export async function getCurrentUserTeams(userId: number) {
 ...
 */
export async function addUserToTeam(userId: number, teamId: number, startDate?: Date) {
 ...
 */
export async function removeUserFromTeam(userId: number, teamId: number, endDate?: Date) {
